...
kubectl --context docker-desktop create namespace paseo-system --dry-run=client -o yaml |
  kubectl --context docker-desktop apply -f -
kubectl --context docker-desktop label namespace paseo-system pod-security.kubernetes.io/enforce=restricted --overwrite
node --import tsx scripts/identity.ts
kubectl --context docker-desktop apply -f charts/paseo/crds/
helm upgrade --install paseo charts/paseo --kube-context docker-desktop --namespace paseo-system --set-string "image.tag=${image_tag}" --set-string "workspace.image=paseo-workspace:${image_tag}" --wait --timeout 3m
kubectl --context docker-desktop --namespace paseo-system rollout status deployment/paseo-gateway --timeout=3m
kubectl --context docker-desktop --namespace paseo-system apply -f deploy/examples/project.yaml
