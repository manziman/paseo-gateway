...
| Codex CLI, official npm package (gateway refresh authority and workspace runtime) | 0.156.1 | Apache-2.0 in its package metadata and [versioned source license](https://github.com/openai/codex/blob/rust-v0.156.1/LICENSE). OpenAI's [self-hosted environment guidance](https://developers.openai.com/api/docs/guides/agents-api/environments/self-hosted) shows installing the CLI in a container; each operator supplies their own credentials. Codex is not endorsed by or affiliated with this project. |
